    ...
    private var header: some View {
        HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: 5) {
                Text(session.server.name).font(.system(size: 34, weight: .bold, design: .rounded))
                HStack(spacing: 8) {
            ...
            HStack(alignment: .top, spacing: 18) {
                VStack(alignment: .leading, spacing: 4) {
                    HStack(alignment: .firstTextBaseline, spacing: 2) {
                        Text("\(snapshot.healthScore)").font(.system(size: 56, weight: .medium, design: .rounded))
...
private struct MetricColumn: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(value).font(.system(size: 18, weight: .semibold, design: .rounded)).lineLimit(1).minimumScaleFactor(0.65)
            Sparkline(points: points).stroke(AppTheme.accent, style: StrokeStyle(lineWidth: 2, lineCap: .round, lineJoin: .round)).frame(height: 42)
...
private struct StoragePanel: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack { Text("阵列与存储").font(.title3.bold()); Spacer(); Text(snapshot.parityValid ? "Parity 有效" : "需要校验").foregroundStyle(snapshot.parityValid ? AppTheme.healthy : AppTheme.warning) }.padding(16)
            VStack(spacing: 8) {
            ...
            ForEach(snapshot.disks) { disk in
                ...
                DiskRow(disk: disk).padding(.horizontal, 16)
            }
        }.appSurface()
    }
}
...
private struct DiskRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Image(systemName: "internaldrive.fill").font(.title2).foregroundStyle(disk.status == .warning ? AppTheme.warning : AppTheme.secondaryText)
...
private struct RunningPanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            Text("正在运行").font(.title3.bold()).padding(16)
...
private struct ServiceRow: View {
    ...
    var body: some View {
        HStack(spacing: 14) {
            Image(systemName: icon).font(.title2).foregroundStyle(AppTheme.accent).frame(width: 32)
...
private struct WarningRow: View {
    ...
    var body: some View {
        ...
        }.padding(16).background(AppTheme.warning.opacity(0.08)).clipShape(RoundedRectangle(cornerRadius: 16)).overlay(RoundedRectangle(cornerRadius: 16).stroke(AppTheme.warning.opacity(0.18)))
    }
}
